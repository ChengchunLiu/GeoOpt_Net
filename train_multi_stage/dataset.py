...
# ----------------- Scaffold 辅助函数 -----------------
def _init_public_builder_dataset(config: Config) -> MolecularDataset:
    ds = MolecularDataset.__new__(MolecularDataset)
    ds.split = "single_builder"
    ds.cfg = config
...
def _read_xyz_file(path: str | os.PathLike) -> tuple[list[str], np.ndarray]:
    ...
    if len(symbols) != n_atoms:
        raise ValueError(f"XYZ atom count mismatch in {path}: expected {n_atoms}, found {len(symbols)}")
    return symbols, np.asarray(coords, dtype=np.float64)


def _mol_from_smiles_with_hs(smiles: str) -> Chem.Mol:
...
def _embed_and_mmff_optimize(
    ...
    try:
        ...
        raise RuntimeError(f"Embed error: {exc}") from exc
    if ret != 0 or mol.GetNumConformers() == 0:
        raise RuntimeError(f"Embed failed with code={ret}")

    mmff_props = AllChem.MMFFGetMoleculeProperties(mol, mmffVariant="MMFF94")
    if mmff_props is None:
        raise RuntimeError("MMFF94 not supported")
...
def build_single_data_from_smiles(
    smiles: str,
    atom_coordinate: Optional[str] = None,
    ...
    """Build one PyG Data object with the same molecular helpers used by training."""
    cfg = config or Config.from_dict({})
    ds = _init_public_builder_dataset(cfg)

    mol = _mol_from_smiles_with_hs(smiles)
    mol_symbols = [atom.GetSymbol() for atom in mol.GetAtoms()]

    if input_xyz is None:
        mol, input_coords = _embed_and_mmff_optimize(
            mol,
            random_seed=random_seed,
            mmff_max_iters=mmff_max_iters,
        )
    else:
        xyz_symbols, input_coords = _read_xyz_file(input_xyz)
        if xyz_symbols != mol_symbols:
            raise ValueError(
                "input_xyz element order does not match Chem.AddHs(Chem.MolFromSmiles(smiles)). "
                f"input_xyz={xyz_symbols}; smiles_add_hs={mol_symbols}"
            )

    atom_features = [ds._get_atom_features(atom) for atom in mol.GetAtoms()]
    x = torch.tensor(atom_features, dtype=torch.float)

    atom_edge_index, edge_attr, edge_types, bond_types = ds._get_bond_info(mol)
    edge_type = torch.tensor(edge_types, dtype=torch.long)

    init_dists, init_angles, init_dihedrals, init_dist_idx, init_ang_idx, init_dih_idx = (
        ds._compute_geometrics_from_coords(mol, input_coords)
    )
    angle_edge_index, angle_edge_attr = ds._construct_angle_edges(init_ang_idx, init_angles)
    dihedral_edge_index, dihedral_edge_attr = ds._construct_dihedral_edges(init_dih_idx, init_dihedrals)
    ...
    fields = {
        ...
        "edge_attr": edge_attr,
        "edge_type": edge_type,
        "pos": torch.tensor(input_coords, dtype=torch.float),
        "init_dists": init_dists,
        "init_angles": init_angles,
        "init_dihedrals": init_dihedrals,
        "init_dist_indices": torch.tensor(init_dist_idx, dtype=torch.long),
        ...
        "num_nodes": len(mol_symbols),
        "atom_symbols": mol_symbols,
    }

    if atom_coordinate is None:
        if require_target:
            raise ValueError("atom_coordinate is required when require_target=True")
    else:
        target_coords, target_symbols = ds._parse_coordinates(atom_coordinate)
        if target_coords.shape[0] != len(mol_symbols):
            raise ValueError(
                f"target atom count mismatch: target={target_coords.shape[0]}, mol={len(mol_symbols)}"
            )
        if target_symbols and target_symbols != mol_symbols:
            raise ValueError(
                "target atom order does not match Chem.AddHs(Chem.MolFromSmiles(smiles)). "
                f"target={target_symbols}; smiles_add_hs={mol_symbols}"
            )
        true_dists, true_angles, true_dihedrals, true_dist_idx, true_ang_idx, true_dih_idx = (
            ds._compute_geometrics_from_coords(mol, target_coords.astype(np.float64))
        )
        fields.update(
            {
                "dft_pos": torch.tensor(target_coords, dtype=torch.float),
                "true_dists": true_dists,
                "true_angles": true_angles,
                "true_dihedrals": true_dihedrals,
                "true_dist_indices": torch.tensor(true_dist_idx, dtype=torch.long),
