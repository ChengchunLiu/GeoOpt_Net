...
class GeoGNNConv(MessagePassing):
    ...
    def forward(self, x, edge_index, edge_attr):
        ...
        messages = self.propagate(edge_index, x=x, edge_attr=edge_emb)
        active = torch.zeros(x.size(0), dtype=torch.bool, device=x.device)
        active[edge_index[1].unique()] = True

        out = x.clone()
        updated = x[active] + self.dropout(messages[active])
    ...
    def message(self, x_i, x_j, edge_attr, index, ptr, size_i):
        ...
        alpha = pyg_softmax(score, index, ptr=ptr, num_nodes=size_i)
